...
Public Class RosterViewerForm
    ...
    Dim titlepic(Basic.Titles.Count - 1, 2) As Image
    ' Dim titleholders(NUMBER_OF_TITLES - 1) As String
    Dim noneedlaternames(Basic.Titles.Count - 1) As String
    ...
    Public Structure person
        ...
        Dim id As Integer
    End Structure
    Public Function MaxOf(ByVal L As List(Of Object)) As Object
        Dim B As Object = L.Item(0)
        For i = 1 To L.Count - 1
            B = Math.Max(B, L.Item(i))
        Next
        Return B
    End Function

    Public Function MaxOf(ByVal L()) As Object
        Dim B As Object = L(0)
        For i = 1 To L.Count - 1
            B = Math.Max(B, L(i))
        Next
    ...
    Private Sub title_array_fill()
        ...
        For i = 0 To Basic.Titles.Count - 1
            '===============
            For j = 0 To MData.mywrestlers.Length - 1
                If MData.myimcontainer(j).idstring = Basic.Titles(i).Holder_ID_String Then
                    titlepic(i, 1) = MData.myimcontainer(j).bigimage 'Image.FromFile(Path.Combine(pth, "fullsize", titleholders(i) & ".png"))
                    Exit For
                End If
            Next
            '================
    ...
    Private Sub load_panel()
        ...
        ReDim Preserve a(MData.mywrestlers.Length - 1)
        For i = 0 To a.Length - 1
            a(i).id_string = MData.mywrestlers(i).id_string
            a(i).Name = MData.mywrestlers(i).name
        Next
        ...
        For i = 0 To numofel
            ...
            pics(i).Image = MData.myimcontainer(i).smallimage
            'pics(i).Visible = True
            pics(i).InitialImage = Nothing
            pics(i).WaitOnLoad = False
    ...
    Public Function filetoarray_oftitles(ByVal foldername As String)
        ...
        Dim FinalArray(Basic.Titles.Count - 1, 2) As String
        Dim separator() As String = {"%"}
        For i = 0 To Basic.Titles.Count - 1
            If File.Exists(Path.Combine(foldername, i & ".txt")) Then
            ...
            Else
                FinalArray(i, 0) = "Title desc"

                FinalArray(i, 1) = "Title name"
            End If
    ...
    Private Sub Form1_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ThisPromotionTitles = 0
        For j = 0 To Basic.Titles.Count - 1
            If Basic.Titles(j).Holder_ID_String = "" Then Basic.Titles(j).Holder_ID_String = "9999"
            For i = 0 To MData.mywrestlers.Length - 1
                If MData.mywrestlers(i).id_string = Basic.Titles(j).Holder_ID_String Then
                    noneedlaternames(j) = MData.mywrestlers(i).name

                    If MData.mywrestlers(i).Promotion = Basic.Promotion Then
                ...
                End If
            Next
            If Basic.Titles(j).Promotion = Basic.Promotion Then
                ThisPromotionTitles += 1
            End If
        Next
        ...
        load_panel()
        init_selector_pic_array(ThisPromotionTitles - 1)
        Title_Won()
    ...
    Private Function normal_return_pic(ByVal path As String) As Object
        Dim num As Integer = MData.mywrestlers.Length - 1
        ...
        For i = 0 To num
            picpath = path & "\" & MData.mywrestlers(i).id_string & ".jpg"
            ...
            'End If
            obj(i).id_string = MData.mywrestlers(i).id_string
            obj(i).Name = MData.mywrestlers(i).name
        Next
    ...
    Private Sub add_pics_to_panel(ByVal noe As Integer)
        ...
        For i = 0 To Int(noe / 5) + 1
            ...
            For j = 0 To 5
                If (MData.mywrestlers(count).id_string) <> "9999" And MData.mywrestlers(count).Promotion = Basic.Promotion Then
...
111:    Dim nullpb As System.Windows.Forms.PictureBox
    ...
    Private Sub init_selector_pic_array(ByVal count As Integer)
        ...
        ReDim selectorToReal(selectors.Length - 1)
        Dim cnt As Integer
        For i = 0 To Basic.Titles.Count - 1
            If Basic.Titles(i).Promotion = Basic.Promotion Then
                selectorToReal(cnt) = i
                cnt += 1
            End If
        Next
    ...
    Public Sub open_About_Window(ByVal j As Integer)
        ...
        About_Wrestler_Array(k).Wrestler_id = MData.mywrestlers(j).id_string
        About_Wrestler_Array(k).selected_wrestler = MData.mywrestlers(j)
    ...
    Private Sub podselect(ByVal id As Integer)
        ...
        For i = 0 To MData.mywrestlers.Length - 1
            If (MData.mywrestlers(i).id_string = Basic.Titles(cnterr).Holder_ID_String) And (Basic.Titles(cnterr).Holder_ID_String <> "9999") Then
                ind = i
            End If
        Next

        If MData.mywrestlers(ind).Promotion = Basic.Promotion Then
            wrestler.Cursor = Cursors.Hand
        Else
            wrestler.Cursor = Cursors.Arrow
        End If
    ...
    Private Sub wrestler_Click(sender As System.Object, e As System.EventArgs) Handles wrestler.Click
        ...
        For i = 0 To MData.mywrestlers.Length - 1
            If (MData.mywrestlers(i).id_string = Basic.Titles(cnterr).Holder_ID_String) And (Basic.Titles(cnterr).Holder_ID_String <> "9999") And (MData.mywrestlers(i).Promotion = Basic.Promotion) Then
                open_About_Window(i)
            End If
        Next
    End Sub
    ...
    Private Sub Button1_Click_1(sender As System.Object, e As System.EventArgs)
        MData.Sort_WModel_By_Name(MData.mywrestlers)
        Close()
    End Sub
End Class
