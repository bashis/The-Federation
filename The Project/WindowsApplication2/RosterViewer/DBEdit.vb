...
Public Class DBEdit
    Dim PNames As List(Of String) = New List(Of String) 'to make filling promotion names more simple
    ...
    Sub fillPNames()
        For i = 0 To Basic.Promotions.Count - 1
            PNames.Add(Basic.Promotions(i).Name)
        Next
    End Sub
    ...
    Private Sub save_DB()
        ...
        For i = 0 To k
            Dim MyWrestler As WModel = MData.FindWrestler(DataGridView1.Item(0, i).Value)

            If MyWrestler Is Nothing Then
                ReDim Preserve MData.wrestlers(MData.wrestlers.Length)
                'ReDim Preserve MData.imcontainer(MData.wrestlers.Length)

                MData.wrestlers(MData.wrestlers.Length - 1) = New WModel
                MyWrestler = MData.wrestlers(MData.wrestlers.Length - 1)
            End If
            ...
            With MyWrestler
                ...
                For j = 0 To column_header.Count - 3
                    .stats(j) = DataGridView1.Item(j + 2, i).Value
                Next
                For j = 0 To DefaultConstants.n_of_perks_at_once
                    Dim tmp As Object
                    tmp = DataGridView1.Rows(i).Cells(j + column_header.Count).Value
                    If tmp = Nothing Then
                        .perks(j) = 0
                    Else
                ...
                Next
                Dim tmp2 As Object
                tmp2 = DataGridView1.Rows(i).Cells(DefaultConstants.n_of_perks_at_once + 1 + column_header.Count).Value
                If tmp2 = Nothing Then
                    .style = 0
                Else
                    .style = Array.IndexOf(WStyles.description, tmp2)
                End If
                tmp2 = DataGridView1.Rows(i).Cells(DefaultConstants.n_of_perks_at_once + 2 + column_header.Count).Value
                ...
                Else
                    If tmp2 = "Face" Then
                        .Gimmick.isFace = True
                    Else
                        .Gimmick.isFace = False
                    End If
                End If

                tmp2 = DataGridView1.Rows(i).Cells(DefaultConstants.n_of_perks_at_once + 3 + column_header.Count).Value
                If tmp2 = Nothing Then
                    .Promotion = 0
                Else
                    .Promotion = Array.IndexOf(PNames.ToArray, tmp2)
                End If
                .Finisher = DataGridView1.Item(DefaultConstants.n_of_perks_at_once + 4 + column_header.Count, i).Value
            End With
    ...
    Dim column_header As List(Of String) = New List(Of String) 'String() = {"id_string", "Name", "Wrestling", "Microphone",
    '"Calmness", "Insanity", "Fan-Base", "Push"}
    Private Sub load_headers()
        column_header.Add("id_string")
        column_header.Add("Name")
        For i = 0 To WStats.description.Length - 1
            column_header.Add(WStats.description(i))
        Next

        DataGridView1.AutoGenerateColumns = False
        DataGridView1.ColumnCount = column_header.Count
        ...
        With DataGridView1
            ...
            For i = 0 To column_header.Count - 1
    ...
    Private Sub PopulateDataGridView()
        ...
        With combocolumn(DefaultConstants.n_of_perks_at_once + 3) 'Promotion Column
            ...
            .DefaultCellStyle.NullValue = PNames(0)
            .DisplayStyle = DataGridViewComboBoxDisplayStyle.ComboBox
            .FlatStyle = FlatStyle.Flat
        ...
        For i = 0 To k
            For j = 0 To DefaultConstants.n_of_perks_at_once
                DataGridView1.Rows(i).Cells(j + column_header.Count).Value = Perk.description(wr(i).perks(j))
            Next
            DataGridView1.Rows(i).Cells(DefaultConstants.n_of_perks_at_once + 1 + column_header.Count).Value = WStyles.description(wr(i).style)

            If wr(i).Gimmick.isFace Then
                DataGridView1.Rows(i).Cells(DefaultConstants.n_of_perks_at_once + 2 + column_header.Count).Value = "Face"
            Else
                DataGridView1.Rows(i).Cells(DefaultConstants.n_of_perks_at_once + 2 + column_header.Count).Value = "Heel"
            End If

            DataGridView1.Rows(i).Cells(DefaultConstants.n_of_perks_at_once + 3 + column_header.Count).Value = Basic.Promotions(wr(i).Promotion).Name

            DataGridView1.Rows(i).Cells(DefaultConstants.n_of_perks_at_once + 4 + column_header.Count).Value = wr(i).Finisher
    ...
    Function NextUnusedId() As Integer
        Dim Success As Boolean = False
        Dim ToTry As Integer = 0
        Do While Not Success
            Success = True

            ToTry += 1
            For i = 0 To DataGridView1.Rows.Count - 1
                If WModel.bring_id_to_string_form(ToTry) = DataGridView1.Item(0, i).Value Then
                    Success = False
                    Exit For
                End If
            Next
    ...
    Private Sub AddWrestlerToolStripMenuItem1_Click(sender As System.Object, e As System.EventArgs) Handles AddWrestlerToolStripMenuItem1.Click
        Dim UnusedId As Integer = NextUnusedId()
        Dim n As String = InputBox("What's his name?", "Adding a wrestler", "Wrestler " & UnusedId)
        If n <> "" Then

            k += 1
            Dim addrow As String() = {WModel.bring_id_to_string_form(UnusedId), n, "0", "0", "0", "0", "0", "0"}
