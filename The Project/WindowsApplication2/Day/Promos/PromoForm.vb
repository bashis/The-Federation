...
Public Class PromoForm
    ...
    Dim showLogo As Image = Basic.Promotions(Basic.Promotion).BGImage 'My.Resources.bg_sd
        ...
        For i = 0 To number_of_wrestlers - 1
            ...
            For j = 0 To DefaultConstants.n_of_perks_at_once - 1
                If wr(i).perks(j) = 6 Then 'is FanFav
                    add += RosterViewerForm.random(CInt(wr(i).stats(1) / 2), wr(i).stats(1) * 2)
                Else
                    add += RosterViewerForm.random(CInt(wr(i).stats(1) / 2), wr(i).stats(1))
                End If
            Next
